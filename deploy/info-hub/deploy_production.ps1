param(
	[Alias('Host')][string]$ServerHost = "164.90.180.73",
	[string]$User = "root",
...
scp -i $KeyPath -o IdentitiesOnly=yes -o BatchMode=yes $artifactPath "$User@$ServerHost`:$remoteArtifactPath"
...
ssh -i $KeyPath -o IdentitiesOnly=yes -o BatchMode=yes "$User@$ServerHost" $remoteCommand
