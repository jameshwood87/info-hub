...
const TOWNS = [
	{
		...
		images: [['/blog-img/marbella-events.jpg', 'An open-air summer concert crowd at night'], ['/blog-img/art-gallery-exhibition.jpg', 'Paintings on display in an art gallery']],
	...
	},
	{
		...
		images: [['/blog-img/estepona-events.jpg', 'A festival crowd at an open-air night concert'], ['/blog-img/street-market-vegetables.jpg', 'Fresh vegetables on a street market stall']],
	...
	},
	{
		...
		images: [['/blog-img/malaga-events.jpg', 'A music festival crowd under stage lights at night'], ['/blog-img/theatre-stage-performance.jpg', 'Performers in costume on a theatre stage']],
...
Voice: warm, useful, local-expert; write for holiday-makers and residents. Plain hyphens only - NEVER em-dashes or en-dashes. The Spanish must be natural, not a literal translation.`,
	...
	// Never reuse an event image: take the first image in this town's pool that
	// no article already uses. When the pool is exhausted the post keeps the
	// default hero and the log says so, instead of repeating last month's image.
	const pickTownImage = () => {
		const norm = (u) => { let s = String(u || ''); const h = s.indexOf('.propertylist.es'); if (h >= 0) s = s.slice(h + '.propertylist.es'.length); return s.split('?')[0]; };
		let taken;
		try {
			const items = JSON.parse(fs.readFileSync('/opt/info-hub/var/admin/kb-meta.json', 'utf8')).items || {};
			taken = new Set(Object.values(items).map((m) => m && m.featuredImageUrl).filter(Boolean).map(norm));
